...
const MOVIES_PER_PAGE = 10; // 2 full rows initially (5 movies per row)
...
function Home() {
  ...
  useEffect(() => {
    const fetchMovies = async () => {
      setLoading(true);
      try {
        const res = await fetch(`http://localhost:4000/api/movies/top-rated`);
  ...
  return (
    <div className="top-rated-container">
      <h2 className="top-rated-heading">Top Rated Movies</h2>
        ...
        {visibleMovies.length > 0 ? (
          visibleMovies.map((movie) => {
            ...
            return (
              <div
                key={movie.Movie_id}
                className="top-rated-movie-card"
                onClick={() => navigate(`/movie/${movie.Movie_id}/${encodeURIComponent(movie.title)}`)}
                style={{ cursor: "pointer" }}
              >
                <img src={posterUrl} alt={movie.title} className="top-rated-movie-poster" />
                <h3>{movie.title}</h3>
